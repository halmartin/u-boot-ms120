...
int do_spi_flash_ptest(unsigned int cs, unsigned long offset, unsigned long len)
{
	void *buf;
	int ret = 0;
	int i;
	...
	if((flash = spi_flash_probe(0, cs, speed, mode)) == NULL)
	{
		printf("Failed to initialize SPI flash at 0:%u\n", cs);
		return 1;
	}
	...
	for(i = 0; i < 2; i++)
	{
		...
		buf = map_physmem(load_addr, len, MAP_WRBACK);
		if (!buf) {
			puts("Failed to map physical memory\n");
			ret = 1;
			goto spi_test_exit;
		...
		memset(load_addr, patten, len);
		
		printf("Write patten 0x%x\n", patten);
		ret = spi_flash_write(flash, offset, len, buf);
		...
		buf = map_physmem((load_addr + len), len, MAP_WRBACK);
		if (!buf) {
			puts("Failed to map physical memory\n");
			goto spi_test_exit;
		}
		...
		printf("Check patten\n");
		if(memcmp(load_addr, (load_addr + len), len) != 0)
		{
			printf("incorrect char 0x%x fail\n", patten);
			goto spi_test_exit;
		}
	}
	...
	printf("SPI test %x(%x) on cs %d OK!\n", offset, len, cs);
	printf("\n");
...
static int do_spi_flash(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	else if (strcmp(cmd, "ptest") == 0)
	{
		...
		ret = sf_parse_len_arg(argv[3], &len);
		if (ret != 1)
			return -1;
		
		ret = do_spi_flash_ptest(cs, offset, len);
		printf("%s\n", ret ? "ERROR" : "OK");
	}
#ifdef CONFIG_CMD_SF_TEST
...
U_BOOT_CMD(
	...
	"sf ptest cs offset [+]len	- test `len' bytes from `offset'\n"
	"				  `+len' round up `len' to block size on chip select `cs'\n"
