...
{
#ifdef CONFIG_MERAKI_DISABLE_MBOOT_TIMER
	if (run_command("gpp clear 5", 0) != CMD_RET_SUCCESS) {
		printf("Failed to change SPI mode\n");
	} else if (run_command("sspi 0 40 02010004bf", 0) != CMD_RET_SUCCESS) {
		printf("Failed to disable multiboot timer\n");
	}
#endif
	printf("\nStarting kernel ...\n\n");
