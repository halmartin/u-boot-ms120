...
int do_mem_test(vu_long	*start, vu_long *end)
{
	vu_long	*addr;
	vu_long	len;
	vu_long	offset;
	 ...
	 */
	printf("    Check for addr bits stuck low or shorted.\n");
	 ...
	 */
	printf("    Check each location and invert it for the second pass.\n");
	for (pattern = 1, offset = 0; offset < num_words; pattern++, offset++) {
		WATCHDOG_RESET();
		temp = start[offset];
		if (temp != pattern) {
			...
			char * const argv[])
{
	...
	for (;;) {
		...
		printf("    Check for address bits stuck high. %08x", 0);
		for (offset = 1; offset < len; offset <<= 1) {
		    temp = start[offset];
		    if (temp != pattern) {
			printf ("\nFAILURE: Address bit stuck high @ 0x%.8lx:"
			...
			printf("\b\b\b\b\b\b\b\b%08x", offset);
		}
		printf("\n");
		start[test_offset] = pattern;
		WATCHDOG_RESET();
		 ...
		 * Check for addr bits stuck low or shorted.
		 */
		printf("    Check for addr bits stuck low or shorted. %08x", 0);
		for (test_offset = 1; test_offset < len; test_offset <<= 1) {
		    start[test_offset] = anti_pattern;
			...
			printf("\b\b\b\b\b\b\b\b%08x", test_offset);
		    start[test_offset] = pattern;
		}
		printf("\n");
		 ...
		 * Check each location and invert it for the second pass.
		 */
		printf("    Check each location and invert it for the second pass. %08x", 0);
		for (pattern = 1, offset = 0; offset < num_words; pattern++, offset++) {
		    WATCHDOG_RESET();
		    temp = start[offset];
		    if (temp != pattern) {
		 ...
		 * Check each location for the inverted pattern and zero it.
		 */
		printf("    Check each location for the inverted pattern and zero it. %08x", 0);
		for (pattern = 1, offset = 0; offset < num_words; pattern++, offset++) {
		    WATCHDOG_RESET();
		    anti_pattern = ~pattern;
		    temp = start[offset];
