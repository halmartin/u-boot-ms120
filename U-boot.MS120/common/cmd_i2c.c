...
void tesla_i2c_init24PD(void)
{
	uchar	byte;	
	byte = 0x05;
	i2c_write(0x2e, 0x40, 1, &byte, 1);	/* config enable monitoring  */
	byte = 0x71;
	i2c_write(0x2e, 0x78, 1, &byte, 1);	/* Pin 10 (PWM2/ SMBALERT) is configured as an SMBALERT interrupt output ; TACH1~4  DC bit enable */
	byte = 0x37;
	i2c_write(0x2e, 0x67, 1, &byte, 1);	/* Set Remote 1 Tmin 55C */
	byte = 0x2d;
	i2c_write(0x2e, 0x68, 1, &byte, 1);	/* Set Local Tmin 45C */
	byte = 0x2d;
	i2c_write(0x2e, 0x69, 1, &byte, 1);	/* Set Remote 2 Tmin 45C */
	byte = 0xac;
	i2c_write(0x2e, 0x5f, 1, &byte, 1);	/* Set Remote 1 Trange 20c  and enable HF */
	byte = 0xbc;
	i2c_write(0x2e, 0x60, 1, &byte, 1);	/* Set LocalTrange 26.67c and enable HF */
	byte = 0xac;
	i2c_write(0x2e, 0x61, 1, &byte, 1);	/* Set Remote 2 Trange 20c and enable HF */
	byte = 0xe0;
	i2c_write(0x2e, 0x62, 1, &byte, 1);	/* Fan(PWM1~3) will spin at min duty cycle even Tsens is below Tmin ;Enable SYNC on TACH2~4 to PWM3 */
	byte = 0x00;
	i2c_write(0x2e, 0x64, 1, &byte, 1);	/* PWM 1 min 0% */
	byte = 0x80;
	i2c_write(0x2e, 0x66, 1, &byte, 1);	/* PWM 3 min 50% */
	byte = 0xf4;
	i2c_write(0x2e, 0x38, 1, &byte, 1);	/* PWM 1 max 95% */
	byte = 0xf3;
	i2c_write(0x2e, 0x3a, 1, &byte, 1);	/* PWM 3 max 95% */
	byte = 0x02;
	i2c_write(0x2e, 0x5c, 1, &byte, 1);	/* PWM1 controlled by Remote 2 */
	byte = 0x02;
	i2c_write(0x2e, 0x5e, 1, &byte, 1);	/* PWM3 controlled by Remote 2 */
	byte = 0x4e;
	i2c_write(0x2e, 0x6a, 1, &byte, 1);	/* Remote 1 THERM temperature limit. => Set to 78C the PWM output remains at 100% until the temperature drops below */
	
	byte = 0x44;
	i2c_write(0x2e, 0x6b, 1, &byte, 1);	/* Local THERM temperature limit. => Set to 68C the PWM output remains at 100% until the temperature drops below */
	byte = 0x44;
	i2c_write(0x2e, 0x6c, 1, &byte, 1);	/* Remote 2 THERM temperature limit. => Set to 68C the PWM output remains at 100% until the temperature drops below */
	
	byte = 0xf5;
    i2c_write(0x2e, 0x4e, 1, &byte, 1); /* Remote 1 Temp Low Limit = -11℃ */
    byte = 0x59 ;
    i2c_write(0x2e, 0x4f, 1, &byte, 1); /* Remote 1 Temp High Limit = 89℃ */
	
	byte = 0xf5;
    i2c_write(0x2e, 0x50, 1, &byte, 1); /* Local Temp Low Limit = -11℃ */
    byte = 0x4d ;
    i2c_write(0x2e, 0x51, 1, &byte, 1); /* Local Temp High Limit = 77℃ */
    byte = 0xf5;
    i2c_write(0x2e, 0x52, 1, &byte, 1); /* Remote 2 Temp Low Limit = -11℃ */
    byte = 0x4b;
    i2c_write(0x2e, 0x53, 1, &byte, 1); /* Remote 2 Temp High Limit = 75℃ */
    byte = 0xd2;
    i2c_write(0x2e, 0x55, 1, &byte, 1); /* TACH 1 fan is running too slowly(<100rpm) or has stalled. */
    byte = 0xd2;
    i2c_write(0x2e, 0x57, 1, &byte, 1); /* TACH 2 fan is running too slowly(<100rpm) or has stalled. */
    byte = 0xd2;
    i2c_write(0x2e, 0x59, 1, &byte, 1); /* TACH 3 fan is running too slowly(<100rpm) or has stalled. */
    byte = 0xd2;
    i2c_write(0x2e, 0x5b, 1, &byte, 1); /* TACH 3 fan is running too slowly(<100rpm) or has stalled. */
	
	
	byte = 0x4d;
	i2c_write(0x2e, 0x44, 1, &byte, 1);	/* 1.03V_M Low Limit 1.03*0.97=0.9991 => Set to */
	byte = 0x52;
	i2c_write(0x2e, 0x45, 1, &byte, 1);	/* 1.03V_M High Limit 1.03*1.03=1.0609  => Set to  */
	byte = 0x56;
	i2c_write(0x2e, 0x46, 1, &byte, 1);	/* 1.03V_S Low Limit 1.03*0.97=0.9991 => Set to */
	byte = 0x5b;
	i2c_write(0x2e, 0x47, 1, &byte, 1);	/* 1.03V_S High Limit 1.03*1.03=1.0609  => Set to  */
	byte = 0xb7;
	i2c_write(0x2e, 0x48, 1, &byte, 1);	/* Vcc Low Limit 3.3*0.97=3.201 */
	byte = 0xc8;
	i2c_write(0x2e, 0x49, 1, &byte, 1);	/* Vcc High Limit 3.3*1.03=3.399 */
	byte = 0x1e;
	i2c_write(0x2e, 0x4a, 1, &byte, 1);	/* 0.8V Low Limit 0.8*0.97=0.776 => Set to 0.776V */
	byte = 0x20;
	i2c_write(0x2e, 0x4b, 1, &byte, 1);	/* 0.8V High Limit 0.8*1.03=0.824 => Set to 0.824V */
	byte = 0xb7;
	i2c_write(0x2e, 0x4c, 1, &byte, 1);	/* 12V Low Limit 12V*0.95=11.4V */
	byte = 0xc9;
	i2c_write(0x2e, 0x4d, 1, &byte, 1);	/* 12V High Limit 12V*1.05=12.6V */
	
    
	byte = 	0x0c;
    i2c_write(0x2e, 0x75, 1, &byte, 1); /* mask =1 */
	
	printf("Initialize SG350X-24PD hw-monitor 0x2e \r\n");
	
} 

void tesla_i2c_initKelpie24P48P(void)
{
	uchar	byte;
	
	printf("Initialize hw-monitor 0x2e \r\n");
	
	byte = 0x05;
	i2c_write(0x2e, 0x40, 1, &byte, 1);	/* config enable monitoring  */
	byte = 0x28;
	i2c_write(0x2e, 0x67, 1, &byte, 1);	/* Set Remote 1 Tmin 60C */
	byte = 0x28;
	i2c_write(0x2e, 0x68, 1, &byte, 1);	/* Set Local Tmin 60C */
	byte = 0x28;
	i2c_write(0x2e, 0x69, 1, &byte, 1);	/* Set Remote 2 Tmin 60C */
	byte = 0xcc;
	i2c_write(0x2e, 0x5f, 1, &byte, 1);	/* Set Remote 1 Trange 16c  and enable HF */
	byte = 0xcc;
	i2c_write(0x2e, 0x60, 1, &byte, 1);	/* Set LocalTrange 16c and enable HF */
	byte = 0xcc;
	i2c_write(0x2e, 0x61, 1, &byte, 1);	/* Set Remote 2 Trange 16c and enable HF */
	byte = 0xe0;
	i2c_write(0x2e, 0x62, 1, &byte, 1);	/* Fan will spin at min duty cycle even Tsens is below Tmin */
	byte = 0x00;
	i2c_write(0x2e, 0x64, 1, &byte, 1);	/* PWM 1 min 35% */
	byte = 0x80;
	i2c_write(0x2e, 0x66, 1, &byte, 1);	/* PWM 3 min 35% */
	byte = 0x00;
	i2c_write(0x2e, 0x38, 1, &byte, 1);	/* PWM 1 max 60% */
	byte = 0xff;
	i2c_write(0x2e, 0x3a, 1, &byte, 1);	/* PWM 3 max 60% */
	byte = 0x02;
	i2c_write(0x2e, 0x5c, 1, &byte, 1);	/* PWM1 controlled by Remote 2 */
	byte = 0x02;
	i2c_write(0x2e, 0x5e, 1, &byte, 1);	/* PWM3 controlled by Remote 2 */
	byte = 0x58;
	i2c_write(0x2e, 0x6a, 1, &byte, 1);	/* Remote 2 THERM temperature limit. => Set to 88C the PWM output remains at 100% until the temperature drops below */
	
	byte = 0x4a;
	i2c_write(0x2e, 0x44, 1, &byte, 1);	/* 2.5V Low Limit 0.75*0.95=0.7125 */
	byte = 0x53;
	i2c_write(0x2e, 0x45, 1, &byte, 1);	/* 2.5V High Limit 0.75*1.05=0.7875 */
	byte = 0x92;
	i2c_write(0x2e, 0x46, 1, &byte, 1);	/* Vccp Low Limit 0.95*0.95=0.9025 */
	byte = 0xa2;
	i2c_write(0x2e, 0x47, 1, &byte, 1);	/* Vccp High Limit 0.95*1.05=0.9975 */
	byte = 0xb7;
	i2c_write(0x2e, 0x48, 1, &byte, 1);	/* Vcc Low Limit 3.3*0.955=3.1515 */
	byte = 0xc9;
	i2c_write(0x2e, 0x49, 1, &byte, 1);	/* Vcc High Limit 3.3*1.045=3.4485 */
	byte = 0x1e;
	i2c_write(0x2e, 0x4a, 1, &byte, 1);	/* 5.0V Low Limit 1.05*0.955=0.9975;(Set to 1.014V) */
	byte = 0x20;
	i2c_write(0x2e, 0x4b, 1, &byte, 1);	/* 5.0V High Limit 1.05*1.045=1.09725;(Set to 1.092V) */
	byte = 0xb6;
	i2c_write(0x2e, 0x4c, 1, &byte, 1);	/* 12V Low Limit 12V*9=10.8 */
	byte = 0xce;
	i2c_write(0x2e, 0x4d, 1, &byte, 1);	/* 12V High Limit 12V*1.1=13.2 */
	
    byte = 	0x01;
    i2c_write(0x2e, 0x78, 1, &byte, 1);
    byte = 0xf5;
    i2c_write(0x2e, 0x4e, 1, &byte, 1);
    byte = 0x4e ;
    i2c_write(0x2e, 0x4f, 1, &byte, 1);
    byte = 0xf5;
    i2c_write(0x2e, 0x50, 1, &byte, 1);
    byte = 0x4e ;
    i2c_write(0x2e, 0x51, 1, &byte, 1);
    byte = 0xf5;
    i2c_write(0x2e, 0x52, 1, &byte, 1);
    byte = 0x53;
    i2c_write(0x2e, 0x53, 1, &byte, 1);
    byte =  0xff;
    i2c_write(0x2e, 0x55, 1, &byte, 1);
    byte = 0xd2;
    i2c_write(0x2e, 0x57, 1, &byte, 1);
    byte = 0xd2;
    i2c_write(0x2e, 0x59, 1, &byte, 1);
    byte = 0xd2;
    i2c_write(0x2e, 0x5b, 1, &byte, 1);
	
}	

#if 0 /* For test Tesla 2.2 */
void tesla_i2c_init48(void)
{
	uchar	byte;
	...
	printf("Initialize hw-monitor 0x2d \r\n");
	byte = 0x05;                                                                 
    i2c_write(0x2d, 0x40, 1, &byte, 1);	/* config enable monitoring  */      
    byte = 0x28;                                                                 
    i2c_write(0x2d, 0x67, 1, &byte, 1);	/* Set Remote 1 Tmin 60C */          
    byte = 0x28;                                                                 
    i2c_write(0x2d, 0x68, 1, &byte, 1);	/* Set Local Tmin 60C */    
    byte = 0xcc;                                                                 
    i2c_write(0x2d, 0x5f, 1, &byte, 1);	/* Set Remote 1 Trange 16c  and enable HF */
    byte = 0xcc;                                                                 
    i2c_write(0x2d, 0x60, 1, &byte, 1);	/* Set LocalTrange 16c and enable HF */
    byte = 0xe0;                                                                 
    i2c_write(0x2d, 0x62, 1, &byte, 1);	/* Fan will spin at min duty cycle even Tsens is below Tmin */
    byte = 0x80;                                                                 
    i2c_write(0x2d, 0x64, 1, &byte, 1);	/* PWM 1 min 35% */                  
    byte = 0xff;                                                                 
    i2c_write(0x2d, 0x38, 1, &byte, 1);	/* PWM 1 max 60% */                  
    byte = 0x02;                                                                 
    i2c_write(0x2d, 0x5c, 1, &byte, 1);	/* PWM1 controlled by Remote 2 */    
    byte = 0x58;                                                                 
    i2c_write(0x2d, 0x6a, 1, &byte, 1);	
    byte = 0xb7;                                                                 
    i2c_write(0x2d, 0x48, 1, &byte, 1);
    byte = 0xc8;                                                                 
    i2c_write(0x2d, 0x49, 1, &byte, 1);
    byte = 0x01;                                                                 
    i2c_write(0x2d, 0x78, 1, &byte, 1);
    byte = 0xf5;                                                                 
    i2c_write(0x2d, 0x4e, 1, &byte, 1);
    byte = 0x4e;                                                                 
    i2c_write(0x2d, 0x4f, 1, &byte, 1);
    byte = 0xf5;                                                                 
    i2c_write(0x2d, 0x50, 1, &byte, 1);
    byte = 0x4e;                                                                 
    i2c_write(0x2d, 0x51, 1, &byte, 1);
    byte = 0xd2;                                                                 
    i2c_write(0x2d, 0x55, 1, &byte, 1);
    byte = 0x4b;                                                                 
    i2c_write(0x2d, 0x74, 1, &byte, 1);
    byte = 0xb9;                                                                 
    i2c_write(0x2d, 0x75, 1, &byte, 1);	
} 

void tesla_i2c_init24(void)
{
	uchar	byte;
	printf("Initialize hw-monitor 0x2e \r\n");	
	byte = 0x05;
...
{
	int		speed;
	uchar	byte, spd, spdl, spdh;

	if(strncmp(argv[1], "set", 3) == 0)
	{
		if(strncmp(argv[2], "speed", 5) == 0)
		{
			if(strncmp(argv[3], "auto", 4) == 0)
			{
				...
				i2c_write(0x2d, 0x64, 1, &byte, 1);	/* PWM 1 min 35% */
				byte = 0x59;
				i2c_write(0x2d, 0x66, 1, &byte, 1);	/* PWM 3 min 35% */
				byte = 0x99;
				i2c_write(0x2d, 0x38, 1, &byte, 1);	/* PWM 1 max 60% */
				byte = 0x99;
				i2c_write(0x2d, 0x3a, 1, &byte, 1);	/* PWM 3 max 60% */
				byte = 0x42;
				i2c_write(0x2d, 0x5c, 1, &byte, 1);	/* PWM1 controlled by Remote 2 */
				byte = 0x42;
				i2c_write(0x2d, 0x5e, 1, &byte, 1);	/* PWM3 controlled by Remote 2 */
			...
			spd = (uchar)(0xff * speed / 100);			
			i2c_set_bus_num(0);	
					
			byte = 0x00;
			i2c_write(0x2e, 0x64, 1, &byte, 1);
			byte = 0x00;
			i2c_write(0x2e, 0x66, 1, &byte, 1);
			byte = 0xff;
			i2c_write(0x2e, 0x38, 1, &byte, 1);
			byte = 0xff;
			i2c_write(0x2e, 0x3a, 1, &byte, 1);
			byte = 0xe2;
			i2c_write(0x2e, 0x5c, 1, &byte, 1);
			byte = 0xe2;
			i2c_write(0x2e, 0x5e, 1, &byte, 1);
			byte = spd;
			i2c_write(0x2e, 0x30, 1, &byte, 1);
			byte = spd;
			i2c_write(0x2e, 0x32, 1, &byte, 1);
			
			byte = 0x00;
			i2c_write(0x2d, 0x64, 1, &byte, 1);
			byte = 0x00;
			i2c_write(0x2d, 0x66, 1, &byte, 1);
			byte = 0xff;
			i2c_write(0x2d, 0x38, 1, &byte, 1);
			byte = 0xff;
			i2c_write(0x2d, 0x3a, 1, &byte, 1);
			byte = 0xe2;
			i2c_write(0x2d, 0x5c, 1, &byte, 1);
			byte = 0xe2;
			i2c_write(0x2d, 0x5e, 1, &byte, 1);
			byte = spd;
			i2c_write(0x2d, 0x30, 1, &byte, 1);
			byte = spd;
			i2c_write(0x2d, 0x32, 1, &byte, 1);
			
			printf("Set Fan speed to %d%% PWM\n", speed);
	...
	else if(strncmp(argv[1], "show", 4) == 0)
	{
		...
		if(strncmp(argv[2], "speed", 5) == 0)
		{
		   ...
		   #if 0  
		    /* For Tesla 2.2 */
			i2c_read(0x2e, 0x28, 1, &spdl, 1);
			i2c_read(0x2e, 0x29, 1, &spdh, 1);
			speed = (spdh << 8) | spdl;
			if((spdl == 0xff) && (spdh == 0xff))
				printf("Fan1 stalled\n");
			else
				printf("Fan1 speed: %d RPM\n", (90000 * 60) / speed);
			i2c_read(0x2e, 0x2a, 1, &spdl, 1);
			i2c_read(0x2e, 0x2b, 1, &spdh, 1);
			speed = (spdh << 8) | spdl;
			if((spdl == 0xff) && (spdh == 0xff))
				printf("Fan2 stalled\n");
			else
				printf("Fan2 speed: %d RPM\n", (90000 * 60) / speed);
			i2c_read(0x2e, 0x2c, 1, &spdl, 1);
			i2c_read(0x2e, 0x2d, 1, &spdh, 1);
			speed = (spdh << 8) | spdl;
			if((spdl == 0xff) && (spdh == 0xff))
				printf("Fan3 stalled\n");
			else
				printf("Fan3 speed: %d RPM\n", (90000 * 60) / speed);
			i2c_read(0x2e, 0x2e, 1, &spdl, 1);
			i2c_read(0x2e, 0x2f, 1, &spdh, 1);
			speed = (spdh << 8) | spdl;
			if((spdl == 0xff) && (spdh == 0xff))
				printf("Fan4 stalled\n");
			else
				printf("Fan4 speed: %d RPM\n", (90000 * 60) / speed);
				
			i2c_read(0x2d, 0x28, 1, &spdl, 1);
			i2c_read(0x2d, 0x29, 1, &spdh, 1);
			speed = (spdh << 8) | spdl;
			if((spdl == 0xff) && (spdh == 0xff))
				printf("Fan5 stalled\n");
			else
				printf("Fan5 speed: %d RPM\n", (90000 * 60) / speed);
		   #else
			/* For SG350X-24PD */
			i2c_read(0x2e, 0x2e, 1, &spdl, 1);
			i2c_read(0x2e, 0x2f, 1, &spdh, 1);
...
#if 0
			i2c_read(0x2d, 0x2a, 1, &spdl, 1);
			i2c_read(0x2d, 0x2b, 1, &spdh, 1);
			speed = (spdh << 8) | spdl;
			if((spdl == 0xff) && (spdh == 0xff))
				printf("Fan2 stalled\n");
			else
				printf("Fan2 speed: %d RPM\n", (90000 * 60) / speed);
			i2c_read(0x2d, 0x2c, 1, &spdl, 1);
			i2c_read(0x2d, 0x2d, 1, &spdh, 1);
			speed = (spdh << 8) | spdl;
			if((spdl == 0xff) && (spdh == 0xff))
				printf("Fan3 stalled\n");
			else
				printf("Fan3 speed: %d RPM\n", (90000 * 60) / speed);
			i2c_read(0x2d, 0x2e, 1, &spdl, 1);
			i2c_read(0x2d, 0x2f, 1, &spdh, 1);
			speed = (spdh << 8) | spdl;
			if((spdl == 0xff) && (spdh == 0xff))
				printf("Fan4 stalled\n");
			else
