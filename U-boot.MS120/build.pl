...
sub HELP_MESSAGE
{
	...
	print "\t-a\t(MERAKI) Make U-Boot for Aikido secure boot\n";
        print "\t-r\t(MERAKI) Make release U-Boot\n";
	print "\n";
...
if (defined $opt_a) {
	system("echo \"#define AIKIDO\" >> include/config.h");
	printf "** AIKIDO **\n";
	$ENV{'AIKIDO'} = 1;
} else {
	$ENV{'AIKIDO'} = 0;
...
if($opt_r eq 1)
{
	system("echo \"#define MERAKI_RELEASE\" >> include/config.h");
	print "** MERAKI RELEASE BUILD **\n";
}
...
if(defined $opt_o)
{
	...
	system("mkdir -p $opt_o/bin_hdr");
		...
		system("cp u-boot.bin $opt_o/$endian/$opt_f/u-boot.bin");
		system("cp u-boot $opt_o/$endian/$opt_f/u-boot");
		system("cp u-boot.srec $opt_o/$endian/$opt_f/u-boot.srec");
        }
