...
static int refresh_settings_from_env(void)
{
	...
	int env_id = get_env_id();
	static IPaddr_t _nc_ip = 0; /* server ip */
	static short _nc_out_port = 0; /* target output port */
	static short _nc_in_port = 0; /* source input port */
	...
	if (env_changed_id != env_id) {
		...
		{
			if((_nc_ip != nc_ip) || (_nc_out_port != nc_out_port) || (_nc_in_port != nc_in_port))
			{
				_nc_ip = nc_ip;
				_nc_out_port = nc_out_port;
				_nc_in_port = nc_in_port;
			    /* force arp request */
