...
#define CONFIG_ECDSA
#define CONFIG_ECP_SECP256R1
#define CONFIG_ECP_SECP384R1
#define CONFIG_FORCE_CONFIG_VERIFY
#define CONFIG_FDT_SIGNATURE_ADDR 0xc130000
...
#define CONFIG_CMD_MERAKI

// Disable multiboot timer just before jumping to kernel
#define CONFIG_MERAKI_DISABLE_MBOOT_TIMER
...
/* global definitions. */
#ifdef AIKIDO
#define CONFIG_SYS_TEXT_BASE		0x00180000     /* The Aikido microloader expects us here */

/* Payload app address.  1MB above U-Boot proper.
 *
 * This MUST be able to be stringified, so it can't contain any
 * arithmetic.
 */
#define PAYLOAD_MEM_ADDR		0x00280000
...
#define CONFIG_SERVERIP		192.168.1.140
#define CONFIG_NETMASK		255.255.255.0
#define ETHADDR			"00:00:00:00:51:81"
#define ETH1ADDR		"00:00:00:00:51:82"
#define ENV_ETH_PRIME		"egiga0"

#define SETUP_MTDPART		"mtdparts=mtdparts=armada-nand:8M(uboot),-(ubi)\0"

#define SETUP_MERAKIBOOT	"meraki_boot=ubi check part.new && meraki_check_upgrade && ubi read $loadaddr part.new && bootm $loadaddr#$fit_config; " \
                                "ubi read $loadaddr part.safe && bootm $loadaddr#$fit_config; ubi read $loadaddr part.old && bootm $loadaddr#$fit_config\0"

// NOTE: the manufacturing image doesn't know about the multiboot
// timer, so we need to disable it before we launch the image.
#define SETUP_MFGBOOT		"mfg_boot=ubi read $loadaddr diagnostic1; setenv bootargs console=ttyS0,115200 ubi.mtd=1 earlyprintk; bootm $loadaddr\0"
#define SETUP_MTDIDS		"mtdids=nand0=armada-nand\0"

#define SETUP_BOOT_CMD		"bootcmd=run led_init; ubi part ubi && run muse_load && run tamdump && run bootselect; run led_red; while true; do sleep 1000; done\0"

#ifndef CONFIG_STANDALONE_LOAD_ADDR
#define CONFIG_STANDALONE_LOAD_ADDR   0xc100000
#endif

#define LED_INIT_CMD		"led_init=i2c mw 32 0.1 40; i2c mw 32 8.1 49; i2c mw 32 1.1 3f; run led_orange\0"
#define LED_OFF_CMD		"led_off=i2c mw 32 2.1 0 3; gpp clear 7; gpp clear 8\0"
#define LED_ORANGE_CMD		"led_orange=i2c mw 32 2.1 0 3; gpp clear 7; gpp set 8\0"
#define LED_WHITE_CMD		"led_white=i2c mw 32 2.1 0 3; gpp set 7; gpp clear 8\0"
#define LED_RED_CMD		"led_red=i2c mw 32 2.1 ff; i2c mw 32 3.1 0 2; gpp clear 7; gpp clear 8\0"
#define LED_GREEN_CMD		"led_green=i2c mw 32 2.1 0; i2c mw 32 3.1 ff; i2c mw 32 4.1 0; gpp clear 7; gpp clear 8\0"
#define LED_BLUE_CMD		"led_blue=i2c mw 32 2.1 0 2; i2c mw 32 4.1 ff; gpp clear 7; gpp clear 8\0"

#define MUSE_LENGTH		0x80000	 // 500k should be plenty, but if you need more there's room for up to 1MB on the flash.
#ifdef AIKIDO
#define MUSE_LOAD_CMD		"muse_load=cp " __stringify(PAYLOAD_MEM_ADDR) " " __stringify(CONFIG_STANDALONE_LOAD_ADDR) " " __stringify(MUSE_LENGTH) "\0"
#else
#define MUSE_LOAD_CMD		"muse_load=nand read " __stringify(CONFIG_STANDALONE_LOAD_ADDR) " 100000 " __stringify(MUSE_LENGTH) "\0"
#endif
#define MERAKI_BOOT_IMG		"meraki_boot_image=normal\0"
#define TAMDUMP_CMD		"tamdump=go " __stringify(CONFIG_STANDALONE_LOAD_ADDR) " tamdump\0"
#define BOOTSELECT_CMD		"bootselect=if go " __stringify(CONFIG_STANDALONE_LOAD_ADDR) " bootselect; then " \
					"if test $meraki_boot_image = mfg; then run mfg_boot; else run meraki_boot; fi; else run meraki_boot; fi\0"
#define DISABLE_MBOOT_CMD	"disable_mboot=gpp clear 5; sspi 0 40 02010004bf\0"
...
/* auto boot*/
#ifdef MERAKI_RELEASE
#define CONFIG_BOOTDELAY 0   /* Autoboot with no abort */
#else
#define CONFIG_AUTOBOOT_KEYED
#define CONFIG_AUTOBOOT_STOP_STR "xyzzy"
#define CONFIG_AUTOBOOT_PROMPT "Enter magic word to prevent autoboot...\n"
...
#define CONFIG_ENV_SPARE_BLOCK		2 /* 	A multiple of the NAND device's block size.	*
...
#define CONFIG_SF_DEFAULT_BUS		CONFIG_ENV_SPI_BUS
	...
	/* Boot from SPI settings */
		...
		#if !defined(MV_NAND)// On Kelpie, environment is never stored in SPI.
...
//	#if defined(MV_NAND_BOOT)
	#if defined(MERAKI_RELEASE)
        	#define CONFIG_ENV_IS_NOWHERE
	#else
		#define CONFIG_ENV_IS_IN_NAND
	#endif

		// Put enviroment at top of non-UBI NAND area.  Since
		// this is only for development usage, we don't care
		// about bad blocks.
		...
		#define CONFIG_ENV_RANGE                0x20000   /* One erase block on the Kelpie Spansion NAND */
		#define CONFIG_ENV_OFFSET		(0x800000 - CONFIG_ENV_RANGE)
		#define CONFIG_ENV_ADDR                 CONFIG_ENV_OFFSET
	#if defined(MV_NAND_BOOT)
		#define MONITOR_HEADER_LEN              0x200
		#define CONFIG_SYS_MONITOR_BASE         0
